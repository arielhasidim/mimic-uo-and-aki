---
title: "Standardizing urine output data for AKI analysis: A multicenter proof of concept study - repreducing results for MIMICdb"
...
m1_adj2 <- glm(
  mortality_30 ~ stage_newcons * (
    admission_age + gender + weight_admit + apsiii + first_stage
  ),
  family = binomial,
  data = (
    akis_all_long_complete %>%
...
mrg_efct_m1_adj2 <- avg_comparisons(
  m1_adj2,
  variables = "stage_newcons",
  comparison = "lnoravg",
  transform = exp
...
rr_table_data_m1_adj2 <- mrg_efct_m1_adj2 %>%
    transmute(
      group = "newcons",
      max_stage = as.factor(dplyr::row_number()),
      estimate = paste0(round(estimate, 2), " (",round(conf.low, 2), "-", round(conf.high, 2), ")"),
    ...
    group,
    max_stage,
    or.adj2 = if_else(is.na(estimate), "1 [Reference]", estimate),
    p.adj2 = case_when(
      is.na(p.value) ~ "NA",
      p.value < 0.001 ~ "<.001",
      .default = as.character(round(p.value, 2))
...
mrg_efct_m2_adj2 <- avg_comparisons(
  m2_adj2,
  variables = "stage_newmean",
  comparison = "lnoravg",
  transform = exp
...
rr_table_data_m2_adj2 <- mrg_efct_m2_adj2 %>%
    transmute(
      group = "newmean",
      max_stage = as.factor(dplyr::row_number()),
      estimate = paste0(round(estimate, 2), " (",round(conf.low, 2), "-", round(conf.high, 2), ")"),
...
m3_adj2 <- glm(
  mortality_30 ~ stage_old * (
    admission_age + weight_admit + gender + apsiii + first_stage
  ),
  family = binomial,
  data = (
    akis_all_long_complete %>%
...
mrg_efct_m3_adj2 <- avg_comparisons(
  m3_adj2,
  variables = "stage_old",
  comparison = "lnoravg",
  transform = exp
...
rr_table_data_m3_adj2 <- mrg_efct_m3_adj2 %>%
    transmute(
      group = "old",
      max_stage = as.factor(dplyr::row_number()),
      estimate = paste0(round(estimate, 2), " (",round(conf.low, 2), "-", round(conf.high, 2), ")"),
...
mimic_kdigo_inter_survival_table_adj <- rr_table_data %>% 
  ...
  left_join(bind_rows(rr_table_data_m1_adj2, rr_table_data_m2_adj2, rr_table_data_m3_adj2)) %>%
  mutate(
    group = case_when(
      group == "newcons" ~ 'UO-Consecutive',
      group == "newmean" ~ 'UO-Average',
  ...
  tab_spanner(label = "Adjusted Model 2", columns = c(or.adj2, p.adj2), id = "adj2") %>%
  tab_footnote(
    footnote = "Model include age, weight, gender and whether diagnosed on admission",
    locations = cells_column_spanners(spanners = "adj1")
  ) %>%
  tab_footnote(
    footnote = "Model include model 1, CCI score and APS-III score at ICU admission",
    locations = cells_column_spanners(spanners = "adj2")
  )
